...
onMounted(async () => {
    const { clientSecret, error: backendError } =  await axios.post('http://localhost:8000/api/paymentIntents').then((res)=>res.data)
...
<template>
  <div class="flex flex-col items-center gap-5">
    ...
    <div>
     ...
     <div>
       <button type="submit" @click="handleSubmit" class="btn rounded-none">Payer</button>
     </div>
  </div>
